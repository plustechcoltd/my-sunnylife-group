<div class="row">
    <div class="col-lg-8 offset-lg-2">
                ...
                @foreach(config('const.gender_types') as $value)
                    <label class="form-check form-check-inline">
                        <input type="radio" class="form-check-input" name="gender"
                               value="{{ $value }}" {{ old('gender', $user->gender) == $value ? 'checked' : '' }}>
        ...
        <div class="mb-3">
           <label class="form-label">{{ __('label.columns.users.avatar_image_path') }}</label>
                <div class="mb-3 {{ $user->avatar_image_path ?? 'd-none' }}">
                    <img id="preview" src="{{ $user->avatar_image_path ? route('admin.users.show_avatar', ['user' => $user->id]) : '' }}" class="w-64px h-64px" alt="">
                </div>
            <input type="file" class="form-control" name="avatar" id="avatar" onchange="previewFile($('#avatar'))">
            <div class="form-text text-muted">Accepted formats: gif, png, jpg. Max file size 2Mb</div>
        </div>
        <div class="mb-3">
            <label class="form-label">{{ __('label.columns.users.language') }}</label>
            <div>
                @foreach(config('const.language_codes') as $language)
                    <label class="form-check form-check-inline">
                        <input type="radio" class="form-check-input" name="language"
                               value="{{ $language }}" {{ old('language', $user->language) == $language ? 'checked' : '' }}>
            ...
            <div class="mb-3">
                <div class="form-group">
                    ...
                    <input name="allowed_ips" id="allowed_ips" type="text"
                           ...
                           value="{{ old('allowed_ips', is_array($user['allowed_ips']) ? implode(",", $user['allowed_ips']) : '') }}"
                    >
