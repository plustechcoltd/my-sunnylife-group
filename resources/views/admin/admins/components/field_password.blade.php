<div class="row">
    <div class="col-lg-8 offset-lg-2">
        <div class="mb-3">
            @if (\Route::current()->getName() == 'admin.profile.edit_password')
                <div class="form-group mb-3">
                    <label class="form-label">{{ __('passwords.current_password') }}</label>
                    <input name="current_password" type="password" placeholder="" class="form-control @if($errors->has('current_password')) is-invalid @endif">
                    @if($errors->has('current_password'))
                        <div class="validation-invalid-label">{{$errors->first('current_password')}}</div>
                    @endif
                </div>
            @endif
