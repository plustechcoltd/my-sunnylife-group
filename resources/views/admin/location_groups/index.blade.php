...
@section('title')
    {{ __('label.tables.location_groups') }}
@endsection

@section('page-header')
    @component('admin.components.breadcrumb')
        @slot('hide_title_breadcrumb')@endslot
        @slot('title') {{ __('label.tables.location_groups') }}{{ __('label.labels.list') }} @endslot
        @slot('subtitle') {{ __('label.tables.location_groups') }} @endslot
        @slot('button')
            <div class="my-auto ms-auto">
                <a href="{{ route('admin.location_groups.create') }}" class="btn btn-info">
...
@section('content')
    <div class="content">
        <div class="card">
            <div class="card-body">
                <table class="table datatable-selection-single" id="location_group-table">
                    <thead>
                    <tr>
                        <th>{{ __('label.columns.common.id') }}</th>
                        <th>{{ __('label.columns.location_groups.name') }}</th>
                        <th>{{ __('label.columns.location_groups.locations_count') }}</th>
...
@section('scripts')
    <script>
      ...
      $(document).ready(function() {
        const config = {
          tableId: 'location_group-table',
          ajaxUrl: "{{ route('admin.location_groups.index') }}",
          columns: [
            {'data': 'id'},
            {'data': 'name'},
        ...
        $(document).on('click', '.delete-button', function(event) {
          ...
          deleteConfirm(form.attr('action'), form.data('id'), '医療グループを削除');
        });
      });
    </script>
@endsection
