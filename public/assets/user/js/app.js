...
const App = function () {
    ...
    const initNavigationActiveMenu = function () {
        var currentPath = location.pathname == "/" ? "index" : location.pathname.substring(1);
        currentPath = currentPath.substring(currentPath.lastIndexOf("/") + 1);
        if (currentPath) {
            // navbar-nav
            if (document.getElementById("navigation-nav")) {
                var a = document.getElementById("navigation-nav").querySelector('[href="' + currentPath + '"]');
                if (a) {
                    a.classList.add("active");
                    var parentCollapseDiv = a.parentElement.parentElement.parentElement.parentElement.parentElement.querySelector('.navbar-nav-link');
                    if (parentCollapseDiv) {
